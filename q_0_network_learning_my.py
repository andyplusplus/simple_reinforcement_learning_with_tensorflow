...
inp_tf = tf.placeholder(shape=[1, 16], dtype=tf.float32)        ##### placeholder
W = tf.Variable(tf.random_uniform([16,4],0,0.01))
Qout = tf.matmul(inp_tf, W)
predict = tf.argmax(Qout,1)

#Below we obtain the loss by taking the sum of squares difference between the target and prediction Q values.
nextQ = tf.placeholder(shape=[1,4],dtype=tf.float32)      ##### placeholder
...
with tf.Session() as sess:
    ...
    for i in range(num_episodes):
        ...
        for j in range(99):
            ...
            a,allQ = sess.run([predict,Qout], feed_dict={inp_tf: inp})
            ...
            Q1 = sess.run(Qout, feed_dict={inp_tf: np.identity(16)[obs_1:obs_1 + 1]})
            ...
            _,W1 = sess.run([updateModel,W], feed_dict={inp_tf: np.identity(16)[obs:obs + 1], nextQ:targetQ})
            rAll += reward
            obs = obs_1
