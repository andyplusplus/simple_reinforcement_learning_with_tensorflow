...
for i in range(num_episodes_2000):
    ...
    for j in range(99):
        ...
        rand_explore = np.random.randn(1, env.action_space.n) * (1. / (i + 1))
        args_q_table = Q_table[obs, :] + rand_explore
        a = np.argmax(args_q_table)  # selected action: 0-3

        obs_1, reward, done, _ = env.step(a) #Get new state and reward from environment

        # Update Q-Table with new knowledge: q(s,a) = r + y(max(q(s', a'))
